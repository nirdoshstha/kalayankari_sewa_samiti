    ...
    <div class="row">
        <div class="col-lg-7">
            <div class="card recent-sales overflow-auto">
                        ...
                        <tbody>
                            @foreach ($data['notices'] as $notice)
                                <tr>
                                    ...
                                    <td>{{ $notice->title }}</td>
                                    <td><img src="{{asset('storage/'.$notice->image)}}" class="img-thumbnail" width="50" /></td>
                                     
                                    <td class="d-sm-flex align-items-center py-4">
                                        <a href="#" data-bs-toggle="modal"
                                            data-bs-target="#view-{{ $notice->id }}"><i
                                                class="fa fa-eye fs-17 px-3 text-info"></i></a>
                                        <a href="#" data-bs-toggle="modal"
                                        data-bs-target="#edit-{{ $notice->id }}"><i
                                            class="fa fa-edit fs-17 px-3 text-info"></i></a>
                                        
                                            <div class="main-toggle-group d-sm-flex align-items-center"> 
                                                <div class="toggle show_hide toggle-md toggle-{{ $notice->status == '0' ? 'success' : ''}} my-1 {{ $notice->status == '0' ? 'on' : 'off' }}" data-id="{{$notice->id}}">
                                                     <span></span>
                                                </div>
                                            </div>
                                                ...
                                                <form action="{{ route($base_route . 'destroy', $notice->id) }}"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                            <a href="#" ><i class="fa fa-trash fs-17 px-3 text-danger delete-confirm" data-id="{{$notice->id}}"></i></a>
                                                </form>
                                        ...
                                        <div class="row row-sm">
                                            ...
                                            <div class="col-md-8">
                                                @if (isset($notice->image))
                                                     <img src="{{ image_path($notice->image) }}" width="80"
                                                         class="img-thumbnail">
                                                 @else
                                    ...
                                    <hr> 
                                        ...
                                        <div class="form-group">
                                            <div class="row row-sm">
                                                ...
                                                <div class="col-md-8">
                                                     <p>{{$notice->createdBy?->name}} ({{$notice->created_at->format('Y-m-d')}})</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <div class="row row-sm">
                                                ...
                                                <div class="col-md-8">
                                                     <p>{{$notice->updatedBy?->name}} ({{$notice->updated_at->format('Y-m-d')}})</p>
                                                </div>
                                            </div>
                                        </div>
                ...
                {{-- //Edit notice Modal Details --}}
                @foreach ($data['notices'] as $notice)
                <div class="modal fade" id="edit-{{ $notice->id }}" tabindex="-1"
                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body"> 
                                <form action="{{ route($base_route.'update',$notice->id) }}" method="POST" class="row g-3 main_form" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="type" value="post"> 
                                    <div class="col-md-6">
                                        <div class="form-floating">
                                            <input type="text" name="title" class="form-control title" value="{{isset($notice) ? $notice->title : ''}}" id="floatingName" placeholder="Album Name">
                                            <label for="floatingName">Title</label>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-floating">
                                            <input type="text" name="sub_title" class="form-control title" value="{{isset($notice) ? $notice->sub_title : ''}}" id="floatingName" placeholder="Album Name">
                                        ...
                                        <label for="" class="form-label">Image</label>
                                            <div class="form-group">
                                                <div class="d-flex justify-content-between">
                                                    ...
                                                    <img src="{{asset('storage/'.$notice->image)}}" width="50" class="img-thumbnail">  
                                                    @else
                                                    <input name="image" class="form-control p-4" type="file"><span class="p-2"></span>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-floating">
                                            <input type="text" name="rank" class="form-control title" value="{{isset($notice) ? $notice->rank : ''}}" id="floatingName" placeholder="Album Name">
                                            <label for="floatingName">Rank</label>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-floating">
                                            <textarea name="description" class="form-control" style="height: 100px;">{{isset($notice) ? $notice->description : ''}}</textarea>
            ...
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">
                        ...
                        @endif
                         {{isset($data['notice']) ? 'Update' : 'Create' }} Page {{$panel ?? ''}}</h5>
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body tabs-menu-body">
                            ...
                            <form action="{{ route($base_route.'update',$data['notice']->id) }}" method="POST" class="row g-3 main_form">
                            @csrf
                            @method('PUT') 
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab1"> 
                                    <div class="col-md-12 mb-3">
                                        <div class="form-floating">
                                            <input type="text" name="title" value="{{ isset($data['notice']) ? $data['notice']->title : old('title')}}" class="form-control" id="title"
                                                placeholder="Title">
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <div class="form-floating">
                                            <input type="text" name="sub_title" value="{{ isset($data['notice']) ? $data['notice']->sub_title : old('sub_title')}}" class="form-control"   
                                        ...
                                        <div class="form-group">                     
                                            <div class="image dropify-wrapper">
                                                @if(!empty($data['notice']->image))
                                                    <img src="{{ asset('storage/' . $data['notice']->image) }}" alt=""
                                                        class="previewImage">
                                                @else
                                ...
                                <div class="tab-pane mb-3" id="tab2">
                                           ...
                                           <textarea name="description" class="form-control"  id="summernote" style="height: 290px;">{{isset($data['notice']) ? $data['notice']->description : old('description')}}</textarea>
                                ...
                                <div class="tab-pane" id="tab4">
                                    <div class="col-md-12 mb-3">
                                        <div class="form-floating">
                                            <input type="text" name="seo_title" value="{{ isset($data['notice']) ? $data['notice']->seo_title : ''}}" class="form-control" id="floatingName"
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <div class="form-floating">
                                            <input type="text" name="seo_keyword" value="{{ isset($data['notice']) ? $data['notice']->seo_keyword : ''}}" class="form-control"   
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <div class="form-floating">
                                            <textarea  name="seo_description" value="" class="form-control"   
                                                placeholder="Seo Description">{{ isset($data['notice']) ? $data['notice']->seo_description : ''}}</textarea>
