                    ...
                    <div class="col-md-6">
                        <div class="left__side-header d-flex">
                            ...
                            @isset(setting()?->phone)
                                ...
                                <i class="fa-solid fa-phone text-success"></i>{{ setting()?->phone ?? ''}}</a>
                                </li>
                            @endisset 
                               
                            @isset(setting()?->mobile)
                                <li><a class="dropdown-item pt-0 text-dark" href="tel: {{ setting()?->mobile ?? ''}}">
                                <i class="fa-brands fa-viber"></i>{{ setting()?->mobile ?? ''}}</a>
                                </li>
                            @endisset
            ...
            <div class="container">
                <div class="logo">
                    <a href="{{route('frontend.index')}}">
                        <img src="{{asset('frontend/assets/image/logo-07.png')}}" width="100%" height="100%" alt="Logo">
                    </a>
                </div>
                <div class="sitenavigation d-flex">
                    ...
                    @foreach (categories() as $category ) 
                        @if($category->subCategories->count()) 
                            <li class="nav-dropdown"><a href="#" class="navbar__links">{{$category->title}} <i
                                        class="fa-solid fa-angle-down"></i></a>
                                <ul>
                                    @foreach ($category->subCategories->where('status','0') as $subcategory )
                                    
                                        <li><a href="/{{$subcategory->slug}}" class="sub__links">{{$subcategory->title}}</a></li>
                        ...
                        @else 
                             <li class="nav-dropdown"><a href="/{{$category->slug}}" class="navbar__links">{{$category->title}}</a> </li>
                    ...
                    @endforeach
                        ...
                        <li class="nav-dropdown"><a href="#" class="navbar__links">Acadamy <i
                                    class="fa-solid fa-angle-down"></i></a>
                            <ul>
                                <li><a href="{{route('frontend.class_information')}}" class="sub__links">Class Information</a></li>
                            </ul>
                        </li>
                        ...
                        <li class="nav-dropdown"><a href="#" class="navbar__links">Downloads <i
                            ...
                            @foreach (downloads() as $download ) 
                                <li><a href="{{asset('storage/'.$download->image)}}" target="_blank" class="sub__links">{{$download->title}}</a></li> 
                            @endforeach
                            </ul>
                        </li>
